...
describe('gameStrategyTags', () => {
  it('keeps backend strategy order and builds highlight badges', () => {
    expect(buildGameStrategyBadges(['ATLAS_SHOT', 'SOLAR_SHOT', 'MOONSHOT', 'SNIPE', 'BIG_CASHOUT'])).toEqual([
  ...
  it('uses position-specific labels for open position badges', () => {
    expect(buildGameStrategyBadges(['ATLAS_SHOT', 'SOLAR_SHOT', 'SMALL_CASHOUT', 'SNIPE'], undefined, 'position')).toEqual([
  ...
  it('builds achieved and target badges for open positions', () => {
    expect(buildPositionStrategyBadges(['ATLAS_SHOT', 'SOLAR_SHOT', 'SMALL_CASHOUT'], ['BIG_CASHOUT', 'SNIPE'])).toEqual([
