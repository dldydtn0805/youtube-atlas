...
describe('HomePlaybackSection', () => {
  ...
  it('allows dragging the mobile player preview from the drag handle', async () => {
    render(
      <HomePlaybackSection
        chartPanelProps={{} as never}
        communityPanelProps={{} as never}
    ...
    const dragLayer = document.querySelector<HTMLElement>('.app-shell__sticky-player-preview-drag-layer');

    await waitFor(() => {
      expect(previewShell?.getAttribute('data-visible')).toBe('true');
    });

    expect(previewShell).not.toBeNull();
    expect(dragLayer).not.toBeNull();

    previewShell!.setPointerCapture = vi.fn();
    previewShell!.releasePointerCapture = vi.fn();
    ...
    fireEvent.pointerMove(previewShell!, {
      clientX: 50,
      clientY: 60,
      pointerId: 1,
    });
    fireEvent.pointerUp(previewShell!, {
      clientX: 50,
      clientY: 60,
      pointerId: 1,
    });

    expect(previewShell?.style.left).toBe('42px');
    expect(previewShell?.style.top).toBe('52px');
  });

  it('allows resizing the mobile player preview from the right edge', async () => {
    render(
      <HomePlaybackSection
        chartPanelProps={{} as never}
        communityPanelProps={{} as never}
    ...
    const previewShell = document.querySelector<HTMLElement>('.app-shell__sticky-player-preview-shell');
    const rightHandle = document.querySelector<HTMLElement>(
      '.app-shell__sticky-player-preview-resize-handle--right',
    );
    ...
    expect(previewShell?.style.getPropertyValue('--sticky-player-preview-width')).toBe('180px');
  });
